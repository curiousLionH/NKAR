...
def set_to_shortcut(Gx,Gy,Cx=0,Cy=0,abs_steer_angle=25,current_angle=0,allow_backward=False,allow_CCW=True):
    Mx=[math.cos(math.radians(-current_angle))*(H/math.tan(math.radians(abs(abs_steer_angle))))+math.sin(math.radians(-current_angle))*H+Cx, #Positive coord x
        math.cos(math.radians(-current_angle))*(H/math.tan(math.radians(-abs(abs_steer_angle))))+math.sin(math.radians(-current_angle))*H+Cx] #Negative coord x
    My=[math.cos(math.radians(-current_angle))*H+math.sin(math.radians(-current_angle))*(H/math.tan(math.radians(abs(abs_steer_angle))))+Cy, #Positive coord y
        math.cos(math.radians(-current_angle))*H+math.sin(math.radians(-current_angle))*(H/math.tan(math.radians(-abs(abs_steer_angle))))+Cy] #Negative coord y
    P=[]
    recommanded_P=[]
    ...
    for i in range(2):
        ...
        if abs(C)>=abs(r):
            ...
            pos_X=(-2*alpha*beta+2*Gx+math.sqrt((2*alpha*beta-2*Gx)**2-4*(beta**2-D**2+Gx**2)*(alpha**2+1)))/(2*alpha**2+2)
            pos_Y=(2*pos_X*(Mx[i]-Gx)+Gx**2-Mx[i]**2+Gy**2-My[i]**2+r**2-D**2)/(-2*(My[i]-Gy))
            ...
            if allow_backward and directed_D<0:
                if allow_CCW and L<0:
                    if len(recommanded_P)==0 or abs(recommanded_P[0]["dist"])+abs(recommanded_P[1]["dist"]) > abs(L)+abs(directed_D):
                        recommanded_P=[{"x":pos_X,"y":pos_Y,"direction":math.degrees(P_theta),"angle":steer,"dist":L},{"x":Gx,"y":Gy,"direction":math.degrees(P_theta),"angle":0,"dist":directed_D}]
                    P.append([{"x":pos_X,"y":pos_Y,"direction":math.degrees(P_theta),"angle":steer,"dist":L},{"x":Gx,"y":Gy,"direction":math.degrees(P_theta),"angle":0,"dist":directed_D}])
                elif L>=0:
                    if len(recommanded_P)==0 or abs(recommanded_P[0]["dist"])+abs(recommanded_P[1]["dist"]) > abs(L)+abs(directed_D):
                        recommanded_P=[{"x":pos_X,"y":pos_Y,"direction":math.degrees(P_theta),"angle":steer,"dist":L},{"x":Gx,"y":Gy,"direction":math.degrees(P_theta),"angle":0,"dist":directed_D}]
                    P.append([{"x":pos_X,"y":pos_Y,"direction":math.degrees(P_theta),"angle":steer,"dist":L},{"x":Gx,"y":Gy,"direction":math.degrees(P_theta),"angle":0,"dist":directed_D}])
            ...
            neg_X=(-2*alpha*beta+2*Gx-math.sqrt((2*alpha*beta-2*Gx)**2-4*(beta**2-D**2+Gx**2)*(alpha**2+1)))/(2*alpha**2+2)
            neg_Y=(2*neg_X*(Mx[i]-Gx)+Gx**2-Mx[i]**2+Gy**2-My[i]**2+r**2-D**2)/(-2*(My[i]-Gy))
            ...
            if allow_backward and directed_D<0:
                if allow_CCW and L<0:
                    if len(recommanded_P)==0 or abs(recommanded_P[0]["dist"])+abs(recommanded_P[1]["dist"]) > abs(L)+abs(directed_D):
                        recommanded_P=[{"x":neg_X,"y":neg_Y,"direction":math.degrees(P_theta),"angle":steer,"dist":L},{"x":Gx,"y":Gy,"direction":math.degrees(P_theta),"angle":0,"dist":directed_D}]
                    P.append([{"x":neg_X,"y":neg_Y,"direction":math.degrees(P_theta),"angle":steer,"dist":L},{"x":Gx,"y":Gy,"direction":math.degrees(P_theta),"angle":0,"dist":directed_D}])
                elif L>=0:
                    if len(recommanded_P)==0 or abs(recommanded_P[0]["dist"])+abs(recommanded_P[1]["dist"]) > abs(L)+abs(directed_D):
                        recommanded_P=[{"x":neg_X,"y":neg_Y,"direction":math.degrees(P_theta),"angle":steer,"dist":L},{"x":Gx,"y":Gy,"direction":math.degrees(P_theta),"angle":0,"dist":directed_D}]
                    P.append([{"x":neg_X,"y":neg_Y,"direction":math.degrees(P_theta),"angle":steer,"dist":L},{"x":Gx,"y":Gy,"direction":math.degrees(P_theta),"angle":0,"dist":directed_D}])
...
def move_to_dst(angle,dist,speed=255,min_dst=MN):
    if round(abs(dist),0)>0:
        global Cangle, Gx, Gy, tmpX, tmpY, num
        notice_time = time()
        CAR.reset_ENC()
        ...
        for interval in range(15,89,15):
            for j in range(1,-2,-2):
                ...
                for p in P:
                    ...
                    for step in p:
                        is_colls=check_collision(step["angle"],step["dist"],max_dist=0,min_dist=0,start_x=post_step["x"],start_y=post_step["y"],current_angle=post_step["direction"])
                    ...
                    if is_colls is not False:
                        ...
                        _,dst_P=set_to_shortcut(tmpX,tmpY,allow_backward=ALLOW_BACK,allow_CCW=False,current_angle=p[1]["direction"],Cx=p[1]["x"],Cy=p[1]["y"])
                        ...
                        for tmpP in dst_P:
                            ...
                            for step in tmpP:
                                ...
                                if step["angle"]==0:
                                    tmpMN=MN
                                is_colls=check_collision(step["angle"],step["dist"],max_dist=0,min_dist=tmpMN,current_angle=post_step["direction"],start_x=post_step["x"],start_y=post_step["y"])
                                post_step=step
                                if is_colls is not False:
                                    break
                            if is_colls is not False:

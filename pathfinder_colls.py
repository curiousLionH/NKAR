...
class Platform:
    ...
    def main(self):    
        ...
        dst,_=set_to_shortcut(allow_backward=self.ALLOW_BACK)
        ...
        while i<len(dst):
            ...
            if i==len(dst)-1:
                min_dst=self.MN
            colls=move_to_dst(dst[i]["angle"],dst[i]["dist"],self.SPEED,min_dst=min_dst)
            ...
            if colls is not True and colls is not None:
                ...
                for interval in range(15,89,15):
                    for j in range(1,-2,-2):
                        ...
                        _,P=set_to_shortcut(x,y,allow_backward=self.ALLOW_BACK,allow_CCW=self.REVERSE_TURN)
                        result_P=[]
                        ...
                        for p in P:
                            ...
                            if is_colls is not False:
                                ...
                                _,dst_P=set_to_shortcut(self.tmpX,self.tmpY,allow_backward=self.ALLOW_BACK,allow_CCW=False,current_angle=p[1]["direction"],Cx=p[1]["x"],Cy=p[1]["y"])
...
# 목적지까지 이동
def move_to_dst(self, angle,dist,speed=255):
    min_dst=self.MN
    if round(abs(dist),0)>0:
        ...
        colls=check_collision(angle,dist,max_dist=self.MX,min_dist=min_dst)
        restart_delay=1
        rst_time=0
        ...
        while True:
            ...
            colls=check_collision(angle,dist-cur_dist,max_dist=self.MX,min_dist=min_dst)
